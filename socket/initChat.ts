...
export const initChat = (io: Server) => {
  io.on("connection", (socket: Socket) => {
     ...
     */
    socket.on("join-lounge", async ({ loungeId, userId }: { loungeId: string; userId: string }) => {
      try {
     ...
     */
    socket.on("typing", ({ loungeId, userId, isTyping }: { loungeId: string; userId: string; isTyping: boolean }) => {
      try {
     ...
     */
    socket.on("mark-multiple-read", async ({ 
      messageIds, 
      userId 
    }: { 
      messageIds: string[]; 
      ...
      try {
        ...
        loungeIds.forEach((loungeId: string) => {
     ...
     */
    socket.on("reconnect", async ({ userId, loungeIds }: { userId: string; loungeIds: string[] }) => {
      try {
        // Rejoin all lounges
     ...
     */
    socket.on("leave-lounge", async ({ loungeId, userId }: { loungeId: string; userId: string }) => {
      try {
     ...
     */
    socket.on("get-message-reads", async ({ messageId }: { messageId: string }) => {
      try {
